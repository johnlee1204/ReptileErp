...
Ext.define('AgileInventory.view.ProductForm', {
	...
	readVariants: function(productId) {
		AERP.Ajax.request({
			url:'/AgileInventory/readVariantsForProduct',
			jsonData:{productId:productId},
			success:function(reply) {
				this.getViewModel().getStore("VariantStore").loadData(reply.data);
			},
			scope:this,
			mask:this
		});
	...
	readOnHand: function(productId) {
		AERP.Ajax.request({
			url:'/AgileInventory/readOnHandForProduct',
			jsonData:{productId:productId},
			success:function(reply) {
				let viewModel = this.getViewModel();
